...
function inputIsValid(req, _res, next) {
  ...
  let res_DateTime = reservation_date + ' ' + reservation_time;
  let reservation_AsDate = new Date(res_DateTime);
  
  

  if (reservation_AsDate && reservation_AsDate.getDay() === 2) {
    return next({
      status: 400,
      message: "The restaurant is closed on Tuesdays. not available for reservations ",
    });
  }
...
function hasValidDate(req, res, next) {
  ...
  if (!dateIsFuture(resDate)) {
    return next({
      status: 400,
      message: `Can't make reservation, reservation must be in the future date and time. `,
    });
  }
