...
function Reservation({ reservation, cancelHandler }) {
  ...
  return (
    ...
    <tr>
      ...
      <td>{ status === "booked" ? (<a href={`/reservations/${reservation.reservation_id}/seat`}><button className="btn btn-sm btn-primary mb-0 me-1" type="submit">Seat</button></a>) :(<p></p>) }
        { status === "booked" ? (<a href={`/reservations/${reservation.reservation_id}/edit`}><button className="btn btn-sm btn-secondary mb-0 me-1">Edit</button></a>) :(<p></p>) }
